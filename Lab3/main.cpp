...
const char mermaidBodyTexFile[] = "images/mermaidTexture.jpg";

// Mermaid object filename
const char mermaidFile[] = "models/mermaid.obj";
...
GLuint spotsTexObj;
GLuint mermaidBodyTexObj;
GLuint mermaidPartsTexObj;
...
void MakeReflectionImage( void )
{
    ...
    double virtualEyePos[3] = { eyePos[0], eyePos[1], 2 * TABLETOP_Z - eyePos[2] };
    glMatrixMode( GL_PROJECTION );  
    glLoadIdentity();        
    glFrustum(TABLETOP_Y1 - virtualEyePos[1], TABLETOP_Y2 - virtualEyePos[1], 
        TABLETOP_X1 - virtualEyePos[0], TABLETOP_X2 - virtualEyePos[0], 
        TABLETOP_Z - virtualEyePos[2], TABLETOP_Z - virtualEyePos[2] + SCENE_RADIUS);

    glMatrixMode( GL_MODELVIEW );
    glLoadIdentity();    
    gluLookAt(virtualEyePos[0], virtualEyePos[1], virtualEyePos[2], eyePos[0], eyePos[1], eyePos[2], 1.0, 0.0, 0.0);
...
// This texture object is for the mermaid body map.

    glGenTextures( 1, &mermaidBodyTexObj);
    glBindTexture( GL_TEXTURE_2D, mermaidBodyTexObj );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

    if ( ReadImageFile( mermaidBodyTexFile, &imageData,
                        &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
    if ( numComponents != 3 )
    {
        printf("%d\n", numComponents);
...
class Face {
    ...
    Face(int numOfSides, int *vertices, int *texcoords, int normal) {
...
void ParseAndDraw( const char *filename ) {
    ...
    for (Face &face : faces) {
        ...
        glBegin(GL_POLYGON);
        for (int i = 0; i < face.numOfSides; i++) {
...
void DrawMermaid( void ) {
    ...
    // draws the mermaid body
    glBindTexture( GL_TEXTURE_2D, mermaidBodyTexObj );

    glPushMatrix();
    glTranslated( TABLETOP_X2, 0.5 * TABLETOP_Y1, 1.7 * TABLETOP_Z );
    glScaled( 0.6, 0.6, 0.6 );
    glRotated(90.0, 1.0, 0.0, 0.0);
    ParseAndDraw(mermaidFile);
    glPopMatrix();
